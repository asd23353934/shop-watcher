# booth-scraper Specification

## Purpose

Defines the scraping behavior for BOOTH, a Japanese doujinshi and creator goods marketplace. The scraper fetches search results via HTTP GET with the `adult=t` parameter to bypass age verification, and parses item data from `li.item-card` data attributes.
