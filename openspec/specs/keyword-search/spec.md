...
Supported platforms: `ruten` (露天拍賣), `pchome` (PChome 24h), `momo` (MOMO購物), `animate` (Animate台灣), `yahoo-auction` (Yahoo拍賣), `mandarake` (Mandarake), `myacg` (買動漫), `kingstone` (金石堂 ACG), `booth` (BOOTH), `dlsite` (DLsite), `toranoana` (Toranoana), `melonbooks` (Melonbooks)
