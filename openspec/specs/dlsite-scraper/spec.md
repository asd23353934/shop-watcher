# dlsite-scraper Specification

## Purpose

Defines the scraping behavior for DLsite, a Japanese digital doujinshi and adult works marketplace. The scraper uses the `maniax` AJAX endpoint with R18 age bypass parameters to fetch newest releases, and parses item data from the JSON response's embedded HTML.
